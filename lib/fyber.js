(function() {
	...
	extend(Fyber.prototype, {
		...
		_next: function next(error, ret) {
			...
			if (!fyber._yieldContinue(ret[1].value))
				fyber.throw(new Error('Incorrect yield using in fyber'));
		},
		...
		start: function () {
			...
			this._generator =
				this._worker instanceof generatorInstance.constructor ? this._worker :
	...
	Fyber.yieldContinues = {
		...
		fyber: function(fyber){
			...
			else this._next(fyber.error, fyber.result);

			return true;
		},

		generator: function(generator){
			if (!(generator instanceof generatorInstance.constructor)) return;
		...
		yieldValue: function(value) {
			...
			value.__cb = this._next.bind(this);
